...
def p_instr_bcond(p):
	'instr : BCOND LABEL'
	cond, addr = p[1]
	p[0] = Bcond(cond, addr, p[2], p.lexer.lineno-1)
...
def p_instr_movwi_imm(p):
	...
	p[0] = Movwi(p[2], p[4], False, p.lexer.lineno-1)
	
def p_instr_movwi_label(p):
	'instr : MOVWI REG COMMA LABEL'
	p[0] = Movwi(p[2], p[4], True, p.lexer.lineno-1)
...
def p_error(p):
	print("ERROR: Syntax error in input on line", p.lexer.lineno-1)
